...
router.get("/auth", auth, (req, res) => {
    res.status(200).json({
        ...
        role: req.userModel.role,
        image: req.userModel.image,
        cart: req.userModel.cart,
        history: req.userModel.history
    });
});
...
router.get("/logout", auth, (req, res) => {
    User.findOneAndUpdate({ _id: req.userModel._id }, { token: "", tokenExp: "" }, (err, doc) => {
        if (err) return res.json({ success: false, err });
...
router.post("/addToCart", auth, (req, res) => {
    ...
    User.findOne({ _id: req.userModel._id },
        (err, userInfo) => {
            ...
            if (duplicate) {
                User.findOneAndUpdate(
                    { _id: req.userModel._id, "cart.id": req.body.productId },
                    { $inc: { "cart.$.quantity": 1 } },
                    { new: true },
                    (err, userInfo) => {
